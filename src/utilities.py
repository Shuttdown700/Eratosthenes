...
def get_primary_root_directories(media_types: list[str]):
    """Returns a list of root directories for primary drives based on media types."""
    import os
    ...
    for media_type in media_types:
        for drive in primary_drives_dict.get(media_type, []):
            drive_letter = get_drive_letter(drive)
            if drive_letter:
                root_directories.append(f"{drive_letter}:/{media_type.capitalize()}/")
    return root_directories

def get_backup_root_directories(media_types: list[str]):
    """Returns a list of root directories for backup drives based on media types."""
    import os
    src_directory = os.path.dirname(os.path.abspath(__file__))
    filepath_drive_hierarchy = os.path.join(src_directory, "..", "config", "alexandria_drives.config")
    drive_config = read_json(filepath_drive_hierarchy)
    ...
    root_directories = []
    for media_type in media_types:
        for drive in backup_drives_dict.get(media_type, []):
            drive_letter = get_drive_letter(drive)
            if drive_letter:
                root_directories.append(f"{drive_letter}:/{media_type.capitalize()}/")
    return root_directories
