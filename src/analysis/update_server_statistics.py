...
def update_server_statistics(bool_update_duration=False, bool_print=False) -> None:
    ...
    if bool_print:
        ...
        print(f'{num_total_files:,} Primary Media Files ({Fore.GREEN}{Style.BRIGHT}{total_size_TB:,.2f} TB{Style.RESET_ALL}, Duration: {total_duration})\n')
        print(f'{Fore.BLUE}{Style.BRIGHT}{num_movie_files:,} BluRay Movies{Style.RESET_ALL} ({size_TB_movies:,} TB, Duration: {duration_movies})')
        print(f'{Fore.MAGENTA}{Style.BRIGHT}{num_uhd_movie_files:,} 4K Movies{Style.RESET_ALL} ({size_TB_uhd_movies:,} TB, Duration: {duration_uhd_movies})')
        print(f'{Fore.GREEN}{Style.BRIGHT}{num_shows:,} TV Shows{Style.RESET_ALL} ({num_show_files:,} Episodes, {size_TB_shows:,} TB, Duration: {duration_shows})')
        print(f'{Fore.RED}{Style.BRIGHT}{num_anime:,} Anime Shows{Style.RESET_ALL} ({num_anime_files:,} Episodes, {size_TB_anime:,} TB, Duration: {duration_anime})')
        print(f'{Fore.CYAN}{Style.BRIGHT}{num_book_files:,} Books{Style.RESET_ALL} ({size_GB_books:,} GB)')
        print(f'{Fore.LIGHTGREEN_EX}{Style.BRIGHT}{num_course_files:,} Course Videos{Style.RESET_ALL} ({size_GB_courses:,} GB, Duration: {duration_courses})')
        print(f'\n{"#" * 10}\n')
